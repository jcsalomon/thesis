...
\newcommand*{\ini}{0}	%{{\mathrm{i}}}
\newcommand*{\veca}{{\mathbf{a}}}
\newcommand*{\vecu}{{\mathbf{u}}}
\newcommand*{\vecx}{{\mathbf{x}}}

\newcommand*{\intt}[1]{\int_{t_\ini}^{t_\fin}\!\!#1}
...
Over the time interval \( t \in [t_\ini, t_\fin] \), for systems as just described,
the following definitions hold:
\begin{defn}[State \& Control Trajectories]
	A history of control-input values  \( \vecu(t) \)
	over the interval \( [t_\ini, t_\fin] \)
	is denoted by \( \vecu \)
...
with a set of predicates (often inequalities) on state or input variables, \eg,
\[
%	\left| x_1(t) \right| < 2
	0 \leq x_1(t) \leq 2.
\]
...
\begin{ex}[Minimum-Time Problems]
	...
	from an arbitrary initial state $\vecx(t_\ini) = \vecx_\ini$
	to a specified target space $S$ in minimum time,
	then the performance measure to be minimized is
	\[
		J = t_\fin - t_\ini = \intt{\dd t},
	\]
	with $t_\fin$ defined as the first instant when $\vecx(t) \in S$.
\end{ex}\noindent
Other common performance measures are minimum-final-deviation from a reference, \ie,
\[
	J =	\left] \mathbf{r}(t_\fin) - \vecx(t_\fin) \right[^2 =
		\sum_{i=1}^n \left[ x_i(t_\fin) - r_i(t_\fin) \right]^2,
\]
